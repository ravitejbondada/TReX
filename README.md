...
- **Mobile date fields:** Add Expense, Goals, and Trips use compact date inputs with reduced icon padding and wider date columns so full mobile date values remain visible. Add Expense keeps Date and Note in one compact row so the save button sits above the bottom nav more reliably.
