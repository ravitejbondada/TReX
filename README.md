...
- **Ledger running balance:** Each visible ledger tile shows a subtle cumulative spend total for the currently filtered/searched/sorted result set. The engine sums the rendered tiles from bottom to top, so changing filters, search, or sort immediately changes the cumulative values. Split groups count as one tile using the grouped visible total.
...
- **Recurring entry point:** Recurring schedules are managed from the sidebar/drawer. Add Expense includes a subtle "Make this recurring" link below the amount panel for users who realize an expense should become a schedule.
- **Split transactions:** Add Expense has a Split across cat toggle in the Category Tag header. Split mode replaces the single category picker with split rows, blocks duplicate categories, and derives the main amount from the row totals. Transaction tiles use a shared layout: name, date + payment, then category chips. Split tiles show every split category chip and use a multi-color stripe with all unique split category colors. The parent delete button warns and deletes the entire split; expand the card to delete an individual split part.
