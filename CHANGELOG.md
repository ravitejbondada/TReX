...
- `js/dashboard.js`, `index.html` — updated heatmap and budget-progress risk colors to green → yellow → amber/orange → rose/red; made heatmap fills/borders ultra-muted; compacted and right-aligned the heatmap legend.
...
- `index.html`, `styles.css` — Add Expense date/note stays in one compact mobile row; goal target/date and trip start/end fields stack on phones and only become two columns at wider breakpoints; date inputs use compact padding/icon sizing so full mobile date text remains visible; Add Expense view has extra bottom padding so the save button clears the bottom nav.
