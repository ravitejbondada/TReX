...
- `index.html`, `styles.css` — Add Expense date/note, goal target/date, and trip start/end fields now stack on phones and only become two columns at wider breakpoints; date inputs use compact padding/icon sizing so full mobile date text remains visible; Add Expense view has extra bottom padding so the save button clears the bottom nav.
...
- Date fields in Add Expense, Goals, and Trips have enough room to show full mobile date values without overflowing their cards.
