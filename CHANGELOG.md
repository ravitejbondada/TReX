...
**Files modified:**
- `js/core.js` — added `_ensurePickerDOM()`, `openCustomPicker(selectEl, titleOverride?)`, `closeCustomPicker()`, `openLedgerSortPicker()`; rewrote `wrapAllSelects()` to attach `mousedown`/`touchstart`/`keydown` interceptors (with `data-picker-attached` guard) on every `app-dropdown` select; simplified `forceDropdownDarkTheme()` (option inline styles no longer needed); MutationObserver retained for auto-wrap on dynamic injection.
...
- `styles.css` — added `#customPickerOverlay`, `#customPickerPanel`, `#customPickerHandle`, `#customPickerTitle`, `#customPickerList`, `.picker-option`, `.picker-option.selected`, `.picker-check` with full dark / light / fossil theme variants.
...
**Behavior:**
- Tapping any dropdown (category, payment, currency, filter selects, recurring frequency, EMI tenure, sort, billing day, etc.) opens the custom bottom-sheet picker on all platforms — no native OS picker appears.
