...
function verifyAndAutocloseGoals() {
    ...
    (state.savingGoals || []).forEach(g => {
        if (!g.completed && g.targetDate && g.targetDate < todayStr) {
            g.completed = true;
            g.completedReason = "expired"; // Flag auto-closed status
            changed = true;
            
            setTimeout(async () => {
                await showGoalConfirm(
...
function renderSavingGoalsDedicated() {
    ...
    if (!completedSection) {
        ...
        completedSection.id = "dedicatedCompletedSavingGoalsContainer";
    ...
    if (activeGoals.length === 0) {
        activeContainer.innerHTML = `<p class="text-[11px] text-slate-500 text-center py-6 italic">${t("No active savings targets defined.", "🥚 No active eggs in the nest yet. Create a goal.")}</p>`;
    } else {
        activeGoals.forEach(g => {
            renderGoalCardInContainer(g, activeContainer);
        });
    }
    ...
    if (completedGoals.length === 0) {
        ...
        completedSection.innerHTML = `
            <div class="flex justify-between items-center px-1 mb-2.5 mt-4">
                <h3 class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest flex items-center gap-1.5">
                    <i data-lucide="archive" class="w-3.5 h-3.5 text-slate-500"></i> Completed Targets
                </h3>
                <span id="completedGoalsSummaryCount"
                    class="text-[9px] font-bold text-slate-400 bg-slate-900/80 border border-slate-800/40 px-2.5 py-0.5 rounded-full">
                    ${completedGoals.length} Completed
                </span>
            </div>
            <div class="space-y-3.5" id="dedicatedCompletedSavingGoalsList"></div>
        `;
        const completedListContainer = document.getElementById("dedicatedCompletedSavingGoalsList");
        if (completedListContainer) {
            completedGoals.forEach(g => {
                renderGoalCardInContainer(g, completedListContainer);
            });
        }
    }

    initLucideIcons(document.getElementById("goalsPanel"));
}

function renderGoalCardInContainer(g, container) {
...
async function saveGoalEdit(id) {
    ...
    if (askToCompleteDueToPastDate) {
        const wantsComplete = await showGoalConfirm(
...
async function deleteGoalContribution(goalId, cid) {
    ...
    const c = g.contributions.find(c => c.id === cid);
    if (!c) return;
    if (!await showGoalConfirm(t(`Remove this contribution of ${state.currencySymbol}${c.amount} from "${g.name}"? This cannot be undone.`, `Remove this nest offering of ${state.currencySymbol}${c.amount} from "${g.name}"? This cannot be undone.`), t("Remove Contribution", "Remove nest offering?"), t("Remove", "Remove offering"), t("Got it", "Dismiss"))) return;
...
async function manuallyCompleteGoal(id) {
    ...
    if (await showGoalConfirm(textMsg, t("Complete Goal?", "Complete Hunt?"), t("Yes, Complete", "Harvest"), t("Got it", "Dismiss"))) {
        goal.completed = true;
...
async function fundSavingGoalDedicated(id) {
    ...
    if (newPercent >= 100 && prevPercent < 100) {
        ...
        if (goal.targetDate) {
            setTimeout(async () => {
                const optComplete = await showGoalConfirm(
...
async function removeSavingGoalDedicated(id) {
    ...
    if (!await showGoalConfirm(t(`Delete goal ${label}? All contributions will be lost.`, `Abandon hunt ${label}? All nest offerings will be lost.`), t("Delete goal?", "Abandon the hunt?"), t("Delete", "Abandon"), t("Got it", "Dismiss"))) return;
...
function switchGoalsTab(tab) {
    ...
    if (tab === "goals") {
        ...
        if (tripsPanel) tripsPanel.classList.add("hidden");
        if (goalsBtn) goalsBtn.className = activeClass;
        if (tripsBtn) tripsBtn.className = inactiveClass;
    } else {
        if (goalsPanel) goalsPanel.classList.add("hidden");
        if (tripsPanel) tripsPanel.classList.remove("hidden");
        if (goalsBtn) goalsBtn.className = inactiveClass;
        if (tripsBtn) tripsBtn.className = activeClass;
        renderTripsList();
    }
    initLucideIcons();
}
...
function openTripDetail(tripId) {
    ...
    tripExpDateEl.onchange = function() {
...
function renderTripExpenses() {
    ...
    if (preItems.length === 0) {
        ...
        if (preEmpty) preEmpty.classList.remove("hidden");
    } else {
        if (preEmpty) preEmpty.classList.add("hidden");
        if (preList) {
    ...
    if (onItems.length === 0) {
        ...
        if (onList) onList.innerHTML = ""; 
        if (onEmpty) onEmpty.classList.remove("hidden");
    } else {
        if (onEmpty) onEmpty.classList.add("hidden");
...
function switchTripTab(tab) {
    ...
    if (onPanel) onPanel.classList.add("hidden"); 
    if (addPanel) addPanel.classList.add("hidden");
    if (preBtn) preBtn.className = inactive; 
    if (onBtn) onBtn.className = inactive; 
    if (addBtn) addBtn.className = inactive;
    if (tab === "pre" && prePanel && preBtn)  { prePanel.classList.remove("hidden"); preBtn.className = active + " bg-violet-600"; }
    if (tab === "on" && onPanel && onBtn)   { onPanel.classList.remove("hidden");  onBtn.className  = active + " bg-amber-600"; }
    if (tab === "add" && addPanel && addBtn)  { addPanel.classList.remove("hidden"); addBtn.className = active + " bg-indigo-600"; }
}
...
async function deleteTripExpense(expenseId) {
    ...
    if (!await showGoalConfirm(t(`Delete ${label}?${exp && exp.type === 'pre' ? ' Its ledger entry will also be removed.' : ''} This cannot be undone.`, `Send ${label} extinct?${exp && exp.type === 'pre' ? ' Its ledger fossil will also be removed.' : ''} This cannot be undone.`), t("Delete expense?", "Send this fossil extinct?"), t("Delete", "Extinct it"), t("Got it", "Dismiss"))) return;
...
async function deleteTripConfirm() {
    ...
    const trip = (state.trips || []).find(t => t.id === activeTripId);
    if (!trip) return;
    if (!await showGoalConfirm(t(`Delete trip "${trip.name}"? All expenses and ledger entries will be removed.`, `Cancel migration "${trip.name}"? All expenses and ledger fossils will be removed.`), t("Delete trip?", "Cancel the migration?"), t("Delete", "Cancel trip"), t("Got it", "Dismiss"))) return;
